  ...
  return (
    <div>
      ...
      <div data-card onPointerDown={() => edit && setSel(null)} style={{ width: dw, height: dh, background: '#fff', borderRadius: bare ? 0 : 14, position: 'relative', overflow: 'hidden', fontFamily: 'Poppins, system-ui, sans-serif', boxShadow: bare ? 'none' : '0 6px 24px rgba(10,20,40,.14)', boxSizing: 'border-box' }}>
        ...
        {/* bottom accent bar — exact match to the DRS deck: flush to the bottom edge, ~4.7% left inset, ~21% wide, thin, square */}
        <div style={{ position: 'absolute', left: dw * 0.047, bottom: 0, width: dw * 0.208, height: Math.max(3, Math.round(dw * 0.009)), background: '#111', borderRadius: 0 }} />
      </div>
    </div>
  );
}
